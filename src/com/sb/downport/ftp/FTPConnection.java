...
public class FTPConnection extends Connection {
   ...
   private static void msg(Object msg){
	   Utils.l("FTPConnection", msg.toString(), false);
   }
   public FTPConnection(String url, String fileName, long startOff, long length, long contentLength, int timeout, ConnectionManager mgr, Object lock, Credential c, animeconfig config) {
      super(url, fileName, startOff, length, contentLength, mgr, lock);
      this.credential = c;
      this.config = config;
   }

   public FTPConnection(Connection.State state, int timeout, ConnectionManager mgr, Object lock, Credential c, animeconfig config) {
