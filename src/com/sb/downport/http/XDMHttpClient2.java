...
import com.sb.downport.Base64;
import com.sb.downport.HTTPUtil;
import com.sb.downport.Logger;
import com.sb.downport.animeconfig;
import com.sb.downport.http.NTLMAutheticator;
import com.sb.downport.http.io.ChunkedInputStream;
...
public class XDMHttpClient2 {
   ...
   public String user;
   public String pass;
   private final static String TAG = "XDMHttpClient2";
   ...
   public XDMHttpClient2(animeconfig config) {
      this.config = config;
   }
   ...
   private void doConnect() throws IOException, AuthenticationException {
      ...
      if(this.protocol.equalsIgnoreCase("http")) {
         ...
         if(this.config.useHttpProxy) {
            ...
            this.proxyType = 30;
            Utils.l(TAG+M, "Host: " + this.host + " Port: " + this.port);
      ...
      //Utils.l(TAG+M,"inputstream: " + in.read());
   ...
   private void initConnection(String uri) throws IOException, URISyntaxException {
	   Utils.l(TAG+"_initConnection","URL: " + uri);
   ...
   private void parseResponse() throws IOException {
	   String M = "_parseResponse";
      int in2;
      if(flag) {
         this.in = this.sock.getInputStream();
         Utils.l("XDMHttpClient2","***AVAIL: " + this.in.available());

         for(int tenc = 0; tenc < 100; ++tenc) {
            in2 = this.in.read();
            if(in2 == 13) {
            	Utils.l("XDMHttpClient2","\'\\r\'");
            } else if(in2 == 10) {
            	Utils.l("XDMHttpClient2","\'\\n\'");
            } else {
            	Utils.l("XDMHttpClient2",(char)in2);
            }
         }
      }

      this.statusLine = HTTPUtil.readLine(this.in);
      Utils.l(TAG + M,"Status: " + this.statusLine + " " + this.statusLine.length());
   ...
   public void sendRequest() throws IOException, AuthenticationException {
	   Utils.l(TAG+"_sendRequest","Getting file..." + this.url);
	   //
      this.requestHeaders.put("accept-encoding", "gzip");//gzip
      ...
      while(prevResponse != 401 || this.user != null && this.user.length() >= 1) {
         ...
         Utils.l("XDMHttpClient2","Check stream 1");
         ...
         Utils.l("XDMHttpClient2","Check stream 3");
         this.out.write(var14.toString().getBytes());
         Utils.l("XDMHttpClient2","Check stream 4");
         this.out.flush();
         Utils.l("XDMHttpClient2","Write Stream: " + var14.toString());
         this.parseResponse();
         prevResponse = this.getResponseCode();
         Utils.l("XDMHttpClient2","SERVER_RESP: " + prevResponse + " " + this.statusLine);
   ...
   private void appendHeaders(StringBuffer requestBuffer, HashMap requestHeaders, boolean tunnel) {
      ...
      Utils.l(TAG+ "_appendHeaders",requestBuffer.toString());
   }
   ...
   public void doTunneling(String host, int port) throws IOException, AuthenticationException {
      ...
      do {
         ...
         Utils.l("XDMHttpClient2",statusLine);
   ...
   public static void main(String[] args) throws Exception {
      animeconfig conf = animeconfig.load(new File(""));
      ...
      while(true) {
         ...
         if(a < 0) {
            client.finishStream();
            Utils.l("XDMHttpClient2","AVAILLLL: " + client.sock.getInputStream().available());
            System.exit(0);
