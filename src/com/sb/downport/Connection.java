...
public abstract class Connection implements Runnable, IXDMConstants {
   ...
   protected Connection(String url, String fileName, long startOff, long length, long contentLength, ConnectionManager mgr, Object lock) {
      ...
      this.startOff = startOff;
      this.lock = lock;
      Utils.l(TAG,"Creating Docwnload");
	  Utils.l(TAG,"URL: " + url);
	  Utils.l(TAG,"staroff: " + startOff);
	  Utils.l(TAG,"length: " + length);
	  Utils.l(TAG,"contentLength: " + contentLength);
	  Utils.l(TAG,"Referrer: " + mgr.referer);
	  Utils.l(TAG,"userAgent: " + mgr.userAgent);
   }
   ...
   public boolean download() {
      this.read = 0L;
      Utils.l(TAG,"starting dwonlaod");
      this.msg1("To download: " + this.length);
      if(this.stop) {
         this.close();
         this.msg1("Returning because STOP");
         return true;
      } else {
         this.chkPause();
         if(this.status != 20) {
            this.msg1("Returning because NOT DOWNLOADING");
            return false;
         } else if(this.in == null) {
            this.msg1("Returning because IN IS NULL");
            return false;
         } else {
            try {
               this.msg1("Init download...");
               this.msg1("Opening file: " + this.fileName);
               ...
               while(!this.stop) {
                  ...
                  if(this.length > -1L && this.downloaded >= this.length) {
                     ...
                     this.msg1("Download complete before: " + this.downloaded + " / " + this.length);
                     this.status = 50;
                     if(this.mgr == null) {
                        return true;
                     }
                     if(this.mgr.donwloadComplete(this)) {
                        this.msg1("called downloadComplete()");
                     ...
                     this.status = 20;
                     this.msg1("called downloadComplete()...");
                     this.msg1("Download complete after: " + this.downloaded + " / " + this.length);
                  }
               ...
               if(this.length < 0L) {
                  ...
                  if(this.stop) {
                     ...
                     this.chkPause();
                     this.close();
                     Utils.l("Connection","Returing false@@@@Error in : " + var17);
                     return false;
                  }
               }
            }
   ...
   public void msg1(Object o) {
	   Utils.l(TAG+"_msg",this.startOff + " :  " + o.toString());
   }
